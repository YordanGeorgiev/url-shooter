#!/bin/bash 
# file: sfw/bash/url-shooter/test-url-shooter.sh
...
doRunTests(){
	...
	while read -r action ; do (
		...
		done < <(find sfw/bash/url-shooter/tests -type f -name '*.sh')
...
doLog(){
   ...
   test -t 1 && echo " [$type_of_msg] `date +%Y.%m.%d-%H:%M:%S` [url-shooter][@$host_name] [$$] $msg "
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the simplistic bash stub url-shooterer
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: sfw/bash/url-shooter/test-url-shooter.sh
