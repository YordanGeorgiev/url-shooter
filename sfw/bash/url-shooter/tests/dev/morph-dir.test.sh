...
doTestMorphDir(){
	...
	sleep 1	
	bash sfw/bash/url-shooter/url-shooter.sh -a morph-dir
	echo now this test should succeed	
	mkdir -p /tmp/sfw/bash	
	cp -rv sfw/bash/url-shooter /tmp/sfw/bash
	export dir_to_morph=/tmp/sfw/bash/url-shooter
	export to_srch=url-shooter
...
#eof file: sfw/bash/url-shooter/tests/dev/morph-dir.test.sh
